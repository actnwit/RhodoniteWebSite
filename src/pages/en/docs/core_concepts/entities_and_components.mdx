...
```ts
const groupEntity = Rn.createGroupEntity(engine); // with TransformComponent, SceneGraphComponent
const meshEntity = Rn.createMeshEntity(engine); // with TransformComponent, SceneGraphComponent, MeshComponent, MeshRendererComponent
```
...
```typescript
const groupEntity = Rn.createGroupEntity(engine);
